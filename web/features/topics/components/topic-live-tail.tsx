...
export function TopicLiveTail({
  ...
  const liveTailViewportRef = useRef<HTMLDivElement | null>(null);
      ...
      <div className="mt-5">
        {records.length === 0 ? (
          ...
          <TopicMessageRecordsView
            records={records}
            emptyTitle="No records buffered"
            emptyCopy="The live tail session is active, but no records have been buffered yet."
            tableViewportRef={liveTailViewportRef}
            tableViewportClassName="max-h-[42rem] overflow-y-auto pr-1"
          />
        )}
      </div>
    </div>
